...
export default function Tree(container, options, onClickCallBack) {
    ...
    e => {
      ...
      if (
        ...
        (target.classList.contains('treejs-checkbox') ||
          target.classList.contains('treejs-label') || target.classList.contains('treejs-label-bold'))
      ) {
...
Tree.prototype.walkUp = function(node, changeState) {
  ...
  if (parent) {
    if (changeState === 'status') {
      ...
      parent.status == 0 && (parent.status = pStatus); // Auto selects the parent if it is not selected.
    } else {
...
Tree.createLiEle = function(node, closed) {
  ...
  (node.id === "Payload" || node.id === "Prepare" ) ? label.classList.add('treejs-label-bold') : label.classList.add('treejs-label');
